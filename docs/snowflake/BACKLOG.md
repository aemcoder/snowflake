...
| **iter-005** | conversion | (verifies #25, Tooling 2 in production) + opportunistic **#56** | **Batch A — afbs regression pass.** 3 pages: `index`, `llm-optimizer`, `brand-concierge`. Setup: branch `iter-05` from `main` (bridge inherited per DEC-016); cargo-cult `content/iter-04/{index,llm-optimizer,brand-concierge}.html` → `content/iter-05/`; **produce `tools/migrate-images.afbs.json`** mapping afbs-02 source image paths → `/media/afbs/<filename>` targets (this manifest doesn't exist yet — needed input for the URL rewriter). Run `node tools/rewrite-content-urls.mjs --in content/iter-05/ --manifest tools/migrate-images.afbs.json --target https://content.da.live/aemcoder/snowflake` to rewrite the `afbs-02--snowflake--aemcoder.aem.page/...` URLs. Then `node tools/da-upload.mjs --what content` + preview/publish. Loop: `node tools/html-diff.mjs --page <slug>` → identify per-module deltas (start point: `docs/snowflake/iterations/baseline-iter-04-html-deltas.md`) → fix canon or content → re-deploy → re-diff until each page <3% drift. Full quality gate per DEC-015. |
