...
|---|---|---|---|
| **#53** | **Per-item `data-slot-attr="<attr>"`** — extend `fillSlot` so a per-item cell value writes to a named attribute on the cloned element. iter-005 confirmed this is the load-bearing missing feature: brands-strip per-logo `style`, acrobat-feature per-card `data-tone` + `<div class="ac-fallback">`, product-section per-card `data-mark` — all 3 index modules + the Semrush `<source src>` need it. PROMOTED from Tier 2 (was iter-008 only). | Index page (afbs) stays at 5.79% drift without it; brands-strip alone is 31% drift. | **iter-006** (next conversion session — replaces Batch B if needed, or lands as a pre-batch bridge tooling session) |
...
| ~~**#25**~~ | ~~URL-rewriter handles cargo-culted iter-N→iter-M prefixes~~ — `tools/rewrite-content-urls.mjs` shipped on `main` with branch-prefix-agnostic regex (any `*--<repo>--<owner>.aem.{page,live}`), manifest-driven mapping, unmapped-URL logging (exit 1). Afbs-specific manifest still needs to land at iter-005 use site. | ~~Batch A reuses iter-02 afbs content.~~ | **SHIPPED** (Tooling 1; but iter-005 made this OBSOLETE — DEC-017 mandates re-extraction from canonical source, not cargo-culting, so the rewriter is only relevant for unusual cases) |
| ~~**#21**~~ | ~~Pre-flight DA token expiry check~~ → SHIPPED in `tools/da-upload.mjs`. Reads `expires_at` (unix millis) from `.hlx/.da-token.json` (Adobe IMS standard); falls back to JWT `exp` claim if absent. Throws on expired with clear `npx aem content clone --path /<...>` re-auth command; warns at <5m remaining; warn-and-continues if neither signal parseable. | iter-04's first canon upload failed 53/53 with bare 401s because the token had silently expired. | **SHIPPED** (Tooling 2) |
| ~~**#26**~~ | ~~`tools/da-upload.mjs` retry on 5xx/429~~ → SHIPPED. `fetchWithRetry` helper wraps both upload + admin-API calls. Up to 3 attempts, exponential backoff (1s/2s/4s), honors `Retry-After` header, retries on 429 + 500/502/503/504 + network errors, short-circuits on non-retryable 4xx. | Today any transient server error fails the file outright; iter-04 had to re-run uploads. | **SHIPPED** (Tooling 2) |
...
|---|---|---|---|
| ~~**#53**~~ | ~~Video `<source src>` slot support~~ → **PROMOTED to Tier 1** (iter-005 discovery: same mechanism unblocks 3 modules on the afbs index page beyond just Semrush video). See Tier 1. | — | **Tier 1** |
| ~~**#28, #34**~~ | ~~Consolidate content-extractor patterns~~ → iter-005's canon-driven `tools/extract-iter05-content.mjs` is the consolidated prototype: reads canon's `[data-slot]` schema, extracts by class match with specificity ordering + consumed-set + nth-of-type fallback. Generalizing it to a single tool that accepts a page config (vs. iter-005's per-page module list) is the remaining work. | Blocks new-page extraction only. | **Tooling 3** (generalize + reuse across all sites) |
...
| **#57** | **bc-final-cta canon variant divergence** — iter-005 removed `__cta` from `canon/modules/final-cta.html` because no source variant on bc / llm / aem / aem-forrester pages used it. But the `bc-prototype` + `bc-bolder` sources DO use `<a class="bc-final-cta__cta">` (no `btn btn--solid-white` classes), and their per-page CSS at `styles/stardust/brand-concierge-{prototype,bolder}-page.css` styles `.bc-final-cta__cta` (gradient + glow + arrow ::after). The current shared canon doesn't render that class anymore. **Two-canon split:** route bc-prototype + bc-bolder to a `final-cta-bold` canon (variant B, just `__cta` no btn classes); leave the existing canon (variant A) for everyone else. Same pattern as iter-04's `acrobat-feature-3up` vs `acrobat-feature`. | iter-07 (bc-prototypes pair) deploys; without it, both pages lose their pink gradient CTA styling. | **iter-07** (BC prototypes batch) |
...
| ~~**Tooling 2**~~ ✅ | bridge | **#21, #26** (Tier 1) | **SHIPPED.** `tools/da-upload.mjs` extended with: (a) token pre-flight via `expires_at` in `.hlx/.da-token.json` with JWT-`exp` fallback; throws on expired, warns at <5m. (b) `fetchWithRetry` wrapper around both PUT + Admin API POST: 3 retries, exponential backoff, honors Retry-After, retries 429+5xx + network errors only. |
| ~~**iter-005**~~ ✅ | conversion | DEC-017 (re-extraction discipline); **#28/#34 prototype** (canon-driven extractor); bridge fixes (decorator `<ul>` unwrap, empty-link drop, text-position, `<picture>` class propagation, `<li>` class preservation, final-cta + training-cta canon cleanup) | **SHIPPED.** Batch A — afbs regression pass. Re-extracted from canonical stardust source per DEC-017 (NOT cargo-culted). Result: brand-concierge **0.80%**, llm-optimizer **2.69%** (both pass <3%); index 5.79% (blocked on #53). 19 binaries to `/media/afbs/`. Promoted **#53 to Tier 1** as the load-bearing remaining feature. See `iterations/005-afbs-batch-a.md`. |
| **iter-006** | conversion or bridge | **#53** (per-item `data-slot-attr`) | **Either a focused Tooling 4 session on `data-slot-attr`** OR **Batch B — AEM Sites quality pass** with #53 work bundled. Decision depends on whether #53 implementation needs >1 session of bridge work to land cleanly. The afbs index page <3% gate also needs #53 retroactively, so closing iter-005's gap can happen either as a sub-task of iter-006 (preferred) or as a separate revisit later. |
| **iter-007** | conversion | — | **Batch C — BC prototypes pair.** 2 pages (proto + bolder; share canons; bolder has bc-marquee). Same closing-pass. |
| **iter-008** | conversion | decide **#35** at close | **Batch D — Semrush.** 1 page; the video-slot use case from spike-001 — but if #53 ships in iter-006, this iteration is purely a per-page conversion exercise rather than a feature delivery. At batch close: decide what of `stardust/` stays in git long-term (#35), now that all 7 pages reference `/media/`. |
...
- Tier 1 (was 6): #8 → T1 ✅, #25 → T1 ✅, #22 → bridge-promotion ✅, #17 → demoted to Tier 3, #21 → T2 ✅, #26 → T2 ✅ — **all Tier-1 drained** ✓
