...
const limit = (n) => {
  ...
  const tick = () => {
    ...
    active += 1;
    fn().then((v) => { active -= 1; resolve(v); tick(); }, (e) => { active -= 1; reject(e); tick(); });
  };
...
async function uploadImages() {
  ...
  for (const mf of manifestFiles) {
    ...
    const entries = Array.isArray(manifest) ? manifest : (manifest.items || manifest.images || manifest.entries || []);
